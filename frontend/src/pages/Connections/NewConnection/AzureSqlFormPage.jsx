...
const DB_TYPE = "azure_sql";

const RBAC_SQL = `CREATE LOGIN duster_readonly WITH PASSWORD = '...';
CREATE USER duster_readonly FOR LOGIN duster_readonly;
ALTER ROLE db_datareader ADD MEMBER duster_readonly;`;

/**
 * Dedicated "Connect Azure SQL" screen, covering the connector's default SQL-auth mode.
 * backend/services/connectors/azure_sql_connector.py also supports Azure AD password,
 * service-principal, and default-credential modes via a `authentication` field — those are
 * left for a follow-up dedicated flow rather than a single form juggling four auth shapes.
 */
export default function AzureSqlFormPage() {
    ...
    <>
      ...
      <div style={{ flex: "1 1 auto", overflowY: "auto", padding: "var(--space-6)", display: "flex", justifyContent: "center" }}>
        <div className="form-page" style={{ width: "100%", display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
          <Breadcrumb segments={[{ label: "Connections", href: "/connections" }, { label: "Connect Azure SQL" }]} />

          <div className="form-head">
            <div className="conn-icon">AZ</div>
            <div>
              <div className="form-title">Connect Azure SQL</div>
            ...
            <form ref={formRef} onSubmit={(e) => e.preventDefault()} style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
              <SheetSection title="Connection details">
                <div className="form-field-stack">
                  <FormField id="az-name" name="name" label="Connection name" defaultValue="azure-sql-connection" />
                  <div className="sheet-field-row">
                    <FormField id="az-host" name="host" label="Server" defaultValue="analytics-prod.database.windows.net" />
                    <FormField id="az-port" name="port" label="Port" defaultValue="1433" />
                  </div>
                  <FormField id="az-database" name="database" label="Database" defaultValue="analytics" />
                  <div className="sheet-field-row">
                    <FormField id="az-username" name="username" label="Username" defaultValue="duster_readonly" />
                    <FormField id="az-password" name="password" label="Password" type="password" />
                  </div>
                  <div className="sheet-field-row">
                    <FormField id="az-encrypt" name="encrypt" label="Encrypt" options={["yes", "no"]} defaultValue="yes" />
                    <FormField id="az-trust-cert" name="trust_server_certificate" label="Trust server certificate" options={["no", "yes"]} defaultValue="no" />
                  </div>
                </div>
              </SheetSection>

              <SheetSection title="Least-privilege setup">
                <p className="form-section-hint">Run this in Azure SQL to grant Duster read-only access scoped to this database.</p>
